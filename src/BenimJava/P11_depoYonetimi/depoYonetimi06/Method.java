    ...
    public static void miktarGuncelle() {
        ...
        if (urunler.containsKey(arananId)) {
           ...
           // System.out.println("id       ismi         ureticisi       miktari       birimi         raf" +
            ...
            //"%d    %-8s       %-14s %3d          %-14s %s"
           // System.out.print(urunler.get(arananId)+urunler.get(arananId).getUrunIsmi()+ urunler.get(arananId).getUretici()+ urunler.get(arananId).getMiktar()+ urunler.get(arananId).getBirim()+ urunler.get(arananId).getRaf());
            //System.out.println("");
...
//*** yukarıdaki sout kullanıcıyı bilgilendirme amaçlı
    ...
    private static void rafaKoy() {
        ...
        if (urunler.keySet().contains(arananId)) {//*** menu
           ...
           try { switch (rfgir) {//*** Bu switch case i koyduk try catch ile beraber
