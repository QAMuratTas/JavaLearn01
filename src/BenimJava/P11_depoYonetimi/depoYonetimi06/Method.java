...
public class Method implements GirisCikisInterface {
    ...
    public static void urunTanimlama() {
        ...
        try {//-->Team09 Aynı ürün isim kontrolu için for döngüsü ile map deki ürün isimlerini karşılaştırdık..
        ...
        System.out.println(P + "Ürün ID :" + urunler.get(id).getId() + "/  Adı :" + urunler.get(id).getUrunIsmi() + "/   Üretici : " + urunler.get(id).getUretici() + "/   Birimi : " + urunler.get(id).getBirim());//-->Team09
        //-->Team09 Kullanıcı bilgilendirme yapıldı
        id++;
    ...
    public static void miktarGuncelle() {
        ...
        //if(!Character.isDigit(arananId)){// -->Team09 gereksiz olmuş -->Team09
...
//-->Team09 yukarıdaki sout kullanıcıyı bilgilendirme amaçlı
    ...
    private static void rafaKoy() {
        ...
        if (urunler.keySet().contains(arananId)) {//-->Team09 menu
            ...
            try {
                switch (rfgir) {//-->Team09 Bu switch case i koyduk try catch ile beraber
            ...
            System.out.println("aradiğiniz urun yok");
        }
    ...
    private static void urunCikis() {
        ...
        if (urunler.keySet().contains(arananId)) {
            System.out.println(P + urunler.get(arananId).getUrunIsmi() + "  miktarı : " + urunler.get(arananId).getMiktar() + " Birim :" + urunler.get(arananId).getBirim());// ***
            System.out.println("Çıkmak istediğiniz  miktari giriniz");//-->Team09
            int cikisMik = scan.nextInt();
            if (urunler.get(arananId).getMiktar() > 0 && (urunler.get(arananId).getMiktar() > cikisMik)) {//-->Team09 miktar olarak hiç yoksa çıkış yapılamaz uyarısı


                urunler.get(arananId).setMiktar(urunler.get(arananId).getMiktar() - cikisMik);
                System.out.println(P + "Mevcut " + urunler.get(arananId).getUrunIsmi() + "miktarı  = " + urunler.get(arananId).getMiktar() + " " + urunler.get(arananId).getBirim());//***
                //***
            } else System.out.println(P + "Ağam olmayan yada olandan fazla ürün çıkartmaya zorluyorsun");//-->Team09
        } else {
            System.out.println("aradiniz urun yok");
        }
