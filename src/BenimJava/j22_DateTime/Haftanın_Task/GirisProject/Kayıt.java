...
public class Kayıt {
    ...
    public ArrayList<Kullanıcı> kayıtAl() {//kullanıcı Class'dan onj uretmek için cons.'a gereken name ve kayıtzamanı
       ...
       //Task 3. step
    public void sansliKullanici( ArrayList<Kullanıcı> kullanıcıKisiler){//kulanıcı list'indeki obj'lerin kayıtZamanını 10 sn
                                                            //az olup olmamasına göre sanslı kullanıcı print edecek
        for (Kullanıcı k:kullanıcıKisiler){//Kullanıcı Class'dan kayıtAl() method ile uretilip kullanıcıKişi list'ine eklene
                                //  objler döngüye alın
    ...
    public void listele (ArrayList<Kullanıcı> kullanıcıKisiler){
        System.out.println(kullanıcıKisiler);//obj 'leri Kuulanıcı Class'daki toString print eder
    }
